...
namespace FreeChoiceDiscipline.Controllers
{
	...
	public class DisciplineController : Controller
	{
		...
		[HttpPost]
		public IActionResult UpdapteDiscipline(string title, Discipline discipline)
		{
			if (title == null || discipline == null)
			{
				_logger.LogError($"Object sent from client is null.");
				return BadRequest(); // View ?
			}

			_repository.Discipline.UpdateDiscipline(title, discipline);

			return StatusCode(201); // View 
		}

		[HttpDelete]
		public IActionResult DeleteDiscipline(string title)
		{
			if (title == null)
			{
				_logger.LogError($"Object sent from client is null.");
			...
			_repository.Discipline.DelateDiscipline(title);
		...
		{
			var disciplines = _repository.Discipline.GetAll(false).ToList();
		...
		public IActionResult GetDiscipline(string title)
		{
			...
			var discipline = _repository.Discipline.FindDisciplineByTitle(title, false);
